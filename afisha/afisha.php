...
<?php
    ...
    foreach ($data['poster'] as $item) {
        ...
        $typeSentenceIngore = ['КАЖДАЯ 7 НОЧЬ - В ПОДАРОК!','РАННЕЕ БРОНИРОВАНИЕ','Я - КРЫМЧАНИН!','ОТЕЛЬ ДЛЯ ДЕТЕЙ БЕСПЛАТНО!','ДНЕВНОЕ ПОСЕЩЕНИЕ','АКЦИИ В РЕСТОРАНАХ','ПОДАРОЧНЫЕ СЕРТИФИКАТЫ','DENTAL CHECK UP', 'CHECK UP'];
